 ...
 * Run from api-gateway directory: cd api-gateway && node ../tests/test-stripe-setup.js
 * Or: cd tests && node test-stripe-setup.js (will change to api-gateway dir)
 */
...
// Change to api-gateway directory to access node_modules
const apiGatewayPath = join(__dirname, "../api-gateway");
chdir(apiGatewayPath);

// Load environment variables from api-gateway/.env
dotenv.config({ path: join(apiGatewayPath, ".env") });
