...
export function downloadHtml(result: JudgeEnvelope): void {
  ...
  const draftStrength = result.prd_score.status === "complete" && scoreReport?.totals
    ? `<section class="section"><p class="eyebrow">Secondary authoring diagnostic</p><div class="score">${scoreReport.totals.final}/${scoreReport.totals.denominator} · Draft strength</div><p><strong>This score does not change the readiness verdict.</strong> PRD Judge remains authoritative.</p><p>Core rubric calibrated on five PRDs. Writing layer unvalidated. PRD Score dimension IDs are independent from the C1–C12 coverage checks later in this report.</p><p>${escapeHtml(scoreReport.anchor_placement)}</p><h2>Improve after the Path to GO</h2><ol>${scoreFixes}</ol><p><strong>Length normalization.</strong> ${escapeHtml(scoreReport.length_normalization.detail)}</p><p><strong>Historical threshold.</strong> ${scoreReport.totals.historical_threshold}/${scoreReport.totals.denominator}; calibration context only.</p><h2>Anchored dimensions</h2>${scoreRows}</section>`
    : `<section class="section"><p class="eyebrow">Secondary authoring diagnostic</p><h2>Draft strength ${result.prd_score.status === "not_scored" ? "was not scored" : "unavailable"}</h2><p>${result.prd_score.status === "not_scored" && scoreReport ? `${escapeHtml(scoreReport.artifact_gate.reason)} No partial score was produced. ` : ""}The readiness judgment is complete and unaffected.</p></section>`;
